...
function PushToTalkConfig() {
  ...
  const optionLabels = options.map(code => {
    ...
    const name = i18nT(BARE_CODE_LABEL_KEY[code])
    ...
    <>
      ...
      <div className="flex flex-col gap-1 pt-2 pb-1">
        ...
        <span className="text-[12px] text-muted">
          {i18nT('pages.settings.sttSettings.ptt_heading_desc')}
        </span>
      </div>

      <SettingsSelect
        label={keyFieldLabel}
        description={i18nT('pages.settings.sttSettings.ptt_key_desc')}
      ...
      {!IS_MAC && (
        <p className="text-[12px] text-warn bg-warn/10 border border-warn/30 rounded-md px-3 py-2 my-1">
