...
export function usePushToTalk(voice: VoiceControls, { disabled }: UsePushToTalkOpts = {}) {
  ...
  const disarm = useCallback((commit: boolean) => {
    ...
    setPhase('idle')
    if (was === 'holding') {
      // Startup still in flight: there is nothing to commit and `stop()` would
      // be a no-op, so abort the startup itself. See `startPendingRef` — this
      // path must NOT depend on the start promise ever settling.
      if (startPendingRef.current) voiceRef.current.cancel()
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // Any non-matching keystroke is also our chance to reconcile: if the bound
      // modifier is no longer physically down, we missed its keyup.
      if (!matchesBinding(e, binding)) {
        if (phaseRef.current !== 'idle' && !stillHeld(e, binding)) disarm(true)
        return
      }
