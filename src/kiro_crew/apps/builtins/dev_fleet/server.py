...
def _manual_restart_command() -> str:
    ...
    Best-effort and platform-shaped: it is shown when Dev Fleet cannot bounce
    the gateway itself, which is precisely the case where we cannot know how the
    gateway was launched. ``kirocrew restart`` is service-aware and the right
    answer for a service install; a bare gateway started from a terminal is
    restarted by the operator the same way they started it.
    """
    if sys.platform == "linux" and shutil.which("systemctl"):
